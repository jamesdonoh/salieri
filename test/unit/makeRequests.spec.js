...
describe('Making component requests', () => {
    ...
    it('should pass endpoint URLs to request-promise', () => {
        ...
        const res = { locals: { contents: [{ endpoint: 'some-url' }, { endpoint: 'other-url' }] } };
        const next = chai.spy();
    ...
    it('should attach fulfilled promise values to the component', (done) => {
        ...
        const makeRequests = getMakeRequests(rpStub);
        const res = { locals: { contents: [{ endpoint: 'some-url' }] } };
        const next = () => {
            expect(res.locals.contents[0].result).to.equal('<<response data>>');
            done();
        };
    ...
    it('should attach rejected promises values to the component', (done) => {
        ...
        const res = { locals: { contents: [{ endpoint: 'some-url' }] } };
        const next = () => {
            expect(res.locals.contents[0].result).to.equal('ERROR');
            done();
        };
